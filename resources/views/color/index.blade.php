						...
						<div class="panel-body">
							<div class="row">
								<div class="col-md-12 support-tickets">
									<a href="/color/create" class="btn btn-sm btn-success btn-block">
										Create Color
									</a>
									<br>
									<table class="table table-bordered table-hover table-condensed">
										<thead>
											<tr>
												...
												<th class="icon text-center">Color Name</th>
										...
										<tbody>
											@foreach($color as $color)
												<tr>
													<td class="icon text-center">{{ $color->id}}</td>
													<td class="description icon text-center" >{{ $color->name}}</td>
													<td class="icon text-center">
														<form role="form" action="/color/{{$color->id}}/delete" method="post">
														<input type="hidden" name="_token" value="{{ csrf_token() }}">
														<a href="{{ URL::to('/color/' . $color->id) . '/edit' }}" class="btn btn-info margin-button"><i class="icon-edit"></i></a>
