						...
						<div class="panel-body">
							<div class="row">
								<div class="col-md-12 support-tickets">
									<a href="/size/create" class="btn btn-sm btn-success btn-block">
										Create Size
									</a>
									<br>
									<table class="table table-bordered table-hover table-condensed">
										<thead>
											<tr>
												...
												<th class="icon text-center">Size Number</th>
										...
										<tbody>
											@foreach($size as $size)
												<tr>
													<td class="icon text-center">{{ $size->id}}</td>
													<td class="description icon text-center" >{{ $size->name}}</td>
													<td class="icon text-center">
														<form role="form" action="/size/{{$size->id}}/delete" method="post">
														<input type="hidden" name="_token" value="{{ csrf_token() }}">
														<a href="{{ URL::to('/size/' . $size->id) . '/edit' }}" class="btn btn-info margin-button"><i class="icon-edit"></i></a>
